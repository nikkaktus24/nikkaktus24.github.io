...
export default class ChooseCapital extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      attempts: 2
  ...
  render() {
    return (
      <div className="choose-capital">
        <h1>Задача на выбор</h1>
        <p>Для продолжения игры тебе нужно выбрать столицу данной страны. Если ты пропустишь задачу, тогда будет ход врага.</p>
        <div className="choose-capital-conditon">{this.state.task.source.condition}</div>
        <ul ref={this.list} onClick={this.onClickVariant} className="choose-capital-variants">
