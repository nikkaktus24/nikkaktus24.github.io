...
import {sample, isElement} from 'lodash';
...
export default class ChooseApproval extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      attempts: 2
  ...
  generate() {
    ...
    return this.setState({
      task: new Exercise(
        {condition: task.condition, variants}
        ,task.answer
        )
    })
  }
  ...
  render() {
    return (
      <div className="choose-approval">
        <h1>Задача на выбор</h1>
        <p>Для продолжения игры тебе нужно выбрать правильное утверждение. Если ты пропустишь задачу, тогда будет ход врага.</p>
        <div className="choose-approval-conditon">{this.state.task.source.condition}</div>
        <ul ref={this.list} onClick={this.onClickVariant} className="choose-approval-variants">
